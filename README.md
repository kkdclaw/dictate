...
запрашивает само системными диалогами, а окно «Состояние и разрешения»
показывает, чего не хватает, и ведёт в нужный раздел Настроек; после выдачи —
кнопка «Перезапустить» там же (или `./daemon.sh restart`).
...
- **Состояние и разрешения…** — окно-панель: служба (готов / грузится / нет
  ...
  кнопками «Запросить»/«Открыть Настройки» и путём к процессу, микрофон и список
  входов, модели (скачана / скачивается X из Y / не скачана — «Скачать»),
  проверка хоткея. Открывается само при первом запуске и когда чего-то не хватает
  (в меню тогда горит ⚠️);
